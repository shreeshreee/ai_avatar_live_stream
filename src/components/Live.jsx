...
function Live() {
    ...
    const [newsContent, setNewsContent] = useState([
        ...
        // { 'headline': 'Medical Fiber Optics Market', 'description': "The global medical fiber optics market is expected to hit $1.7 billion by 2030, fueled by tech advancements, growing healthcare spending, and higher cancer rates. Key players such as Boston Scientific, Coherent, and IPG Photonics are driving market trends." },
        // { 'headline': 'Joint Replacement Devices Market Boom', 'description': "The global Joint Replacement Devices market is projected to reach $27.1 billion by 2031, growing at a 5.7% CAGR. This growth is fueled by the increasing prevalence of joint disorders and the popularity of minimally invasive surgeries." },
    ])
    useEffect(() => {
        getPrNews()
    }, [])

    async function getPrNews() {
        const URL = API + "getPrNews/";
        ...
        <div className={` ${darkTheme ? "bg-[#0e0e0e]" : "bg-white"}`}>
          ...
          Your browser does not support the video tag.
                ...
                <ReactPlayer
                    ...
                    url='https://www.youtube.com/live/Zn8vMrME1po?si=mWODYmQ4LWVxgJAX'
                    // url={video}
                    config={{
                        youtube: {
                            playerVars: {
                        ...
                        <div id="scrollbar-chat" className='w-full justify-center flex flex-wrap p-2 lg:p-10' >
                            {newsContent.map((news, index) => (
                                <div className={` rounded-xl shadow-[0_5px_20px_1px_rgba(0,0,0,0.3)]  ${darkTheme ? "bg-[#1e1e1e]" : "bg-white"} mb-4 md:m-4 p-4 lg:w-[30%] flex flex-col  `}>
                                    ...
                                    <p className={`text-lg text-center mb-4 font-bold ${darkTheme ? "text-white" : "bg-gradient-to-r from-violet-800 to-red-600 bg-clip-text text-transparent"}`}>{index + 1}. {news.title}.</p>
                                    <p className={`${darkTheme ? "text-white" : "text-black"} text-sm leading-7 font-serif`} >{news.summary}</p>
                                </div>
                            ))}
                        </div>
